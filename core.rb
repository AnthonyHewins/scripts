...
def run_cmd(cmd)
	puts "#{" RUN ".bg_green.bold} #{cmd.bold.green}"
	sleep 0.5
	system cmd
end

def run_cmds(*cmds)
	failed_cmd = cmds.find {|cmd| !run_cmd(cmd)}
	return true if failed_cmd.nil?
	err "command failed: #{failed_cmd}"
end

def err(err_msg, exit_code=1)
